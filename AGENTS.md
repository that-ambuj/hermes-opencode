...
Terminal phases: `DONE`, `FAILED`, `KILLED`, `CANCELLED`,
`INVESTIGATION_DONE` (v0.22.0+). The pruner archives terminal agents
after `ARCHIVE_AFTER_SEC = 12h`.
...
## Agent modes (LOAD-BEARING, v0.22.0+)

`Agent.mode` is a string field (one of `state.AGENT_MODES = {"task",
"investigation"}`, default `"task"`) that selects the executor's
expected terminal shape. Set at spawn time via the `oc_spawn` tool's
`mode` parameter; never mutated after that.

### `mode="task"` (default, pre-v0.22 behaviour)

Full pipeline: `EXECUTING -> IDLE_TASK_COMPLETE -> REVIEW_SPAWNING ->
REVIEWING -> REVIEW_DELIVERED -> EXECUTOR_ADDRESSING ->
IDLE_REVIEW_ADDRESSED -> COMMITTING -> PR_OPEN -> DONE`. Executor is
expected to produce a diff and ship a PR. The no-diff branch routes
to `NEEDS_INTERVENTION` via `_enter_no_diff_intervention` (the
v0.21.1 deadlock fix).

### `mode="investigation"` (v0.22.0+)

Skips `REVIEW_SPAWNING / COMMITTING / PR_OPEN` entirely. On idle, the
executor's final assistant text becomes the deliverable and the agent
transitions to the new terminal phase `INVESTIGATION_DONE` via
`_enter_investigation_done(agent, last_text, source=...)`. Worktrees
are cleaned up via the existing `_cleanup_worktrees` helper before
the transition fires.

`_phase_executing` checks `agent.mode == "investigation"` AFTER the
upfront cascade (rate-limit, abort, pending Q/P, status-poll,
awaiting-input). If investigation-mode AND no awaiting-input AND
(READY_FOR_REVIEW sentinel OR 120s idle debounce elapsed):
...
The READY_FOR_REVIEW sentinel bypasses the 120s debounce in
investigation mode just like it does in task mode. The
awaiting-input cascade still wins: a prose question routes to
`AWAITING_HUMAN` exactly like task mode.

The deliverable is stored on `Agent.investigation_deliverable`
(truncated at 8000 chars; the notification body shows the last 2000
chars). `Agent.investigation_done_at` is the archive-trigger
timestamp; the pruner archives `INVESTIGATION_DONE` rows alongside
`DONE` / `CANCELLED` after `ARCHIVE_AFTER_SEC = 12h`.

### Promote path: `oc_promote_to_investigation`

For `mode="task"` spawns that drift into investigation territory by
accident (executor finished an investigation when you asked for a
code change), the no-diff intervention body lists
`oc_promote_to_investigation <agent_id>` as the 4th resolution path
alongside `@<id> <follow-up>`, `oc_cancel`, `oc_retry`. The tool:

1. Refuses on any phase other than NEEDS_INTERVENTION.
2. Reads the last assistant text via `_fetch_last_assistant_text`.
3. Calls `_enter_investigation_done(agent, last_text, source="promote")`.

The source marker on the resulting `investigation_done` notification
distinguishes "promoted from intervention" from organic
investigation completions.

### `oc_ask` (read-only quick lookup)

Sibling to `oc_spawn` but for one-shot questions about a registered
project's code. Does NOT create an agent record. Opens a one-off
opencode session DIRECTLY in `project.repo_path` (no worktree) with
`agent="plan"` (read-only). Hybrid blocking: polls
`list_session_status` + `get_messages` until either (a) the
executor's latest assistant message has `time.completed > send_time`
in which case returns the text inline, or (b) `timeout` seconds
elapse in which case returns `in_flight=true` with an `ask_id` and
spawns `_ask_continue_async` as a background `asyncio.create_task`
that polls for up to 600s and fires an `ask_complete` notification
when the answer arrives. The opencode session is deleted in both
success and timeout-with-eventual-answer branches.

In-flight asks live in the module-level `_PENDING_ASKS: dict[ask_id,
record]` in `tools.py`. Not persisted across gateway restarts;
orphaned opencode sessions from a mid-flight gateway restart are
acceptable (one-off lookups, not production-critical state).

### `oc_project_init` (greenfield)

For brand-new projects: mkdir + `git init -b <base_branch>` + seed
`.gitignore` + `README.md` + initial commit (under the user's git
identity, no overrides per AGENTS.md anti-patterns) + optional `gh
repo create <github_org>/<label> --<visibility> --source . --remote
origin --push` + `projects.add(...)`. Fails fast if `gh` not
authenticated when `github_org` is supplied. Refuses on duplicate
project label and on non-empty paths that aren't already git repos.

For local-only projects, omit `github_org` — step 5 is skipped and
the project_key falls back to `sha256(resolved_path)[:12]` prefixed
`proj_local_` per `projects.project_key_for(...)`.

### When extending agent modes

- Add new mode strings to `state.AGENT_MODES`. Mirror in the
  `oc_spawn` schema's `mode.enum`. Cover with at least one test.
- Add the corresponding terminal phase to `state.PHASES` AND
  `state.TERMINAL_PHASES` if the mode has its own terminus.
- Add an `_enter_<mode>_done(...)` helper that calls
  `_cleanup_worktrees` BEFORE the phase transition and fires a
  notification kind that's in the default `notify_events` set.
- Add the corresponding `*_done_at` timestamp field on `Agent` and
  extend the `_pruner_loop` archive trigger to recognize it.
- Route `_phase_executing` to the new helper at the appropriate
  cascade point. NEVER `return` silently from a mode branch — that's
  the v0.20.x deadlock anti-pattern.
